function [mix, options, errlog] = gmmem_init_htk(gmm_model, x, options)
% GMMEM_INIT_HTK Gaussian Mixture Model training using HTK. Initialization
% is achieved via HInit.
% 
% Prerequisites:
...
% Running on multiple cores or not
if options(19)
    run_parallel = true;
else
    run_parallel = false;
...
gmm_model.vector_type = DATA_KIND;
gmm_model.name = MODEL_NAME;
[htk_model_def, htk_model_macros] = gmm_matlab2htk(gmm_model);
...
% Initialize using HInit
cmd = sprintf('HInit -T 1 -v %f -m 1 -S %s %s',...
    MIN_COVAR,SCP_FILE, htk_model_file);

disp(cmd);
[status, hinit_results] = system(cmd);
%disp(hinit_results);

if ~isempty(regexpi(hinit_results,'Error|USAGE'))
    error(hinit_results);
end
...
if run_parallel
    ...
    %priordisp(cmd);
    [status, par_herest_results] = system(cmd);   
    
    if ~isempty(regexpi(par_herest_results,'Error|USAGE'))
        error(par_herest_results);
    end
else
    cmd = sprintf('HERest -d %s -S %s -M %s -L %s %s',...
        MODEL_DIR, SCP_FILE, MODEL_DIR, DATA_DIR, MODEL_LIST);
    
    for it = 1:niters
        [status, herest_results] = system(cmd);
        
        if ~isempty(regexpi(herest_results,'Error|USAGE'))
            error(herest_results);
        end
    end
end
